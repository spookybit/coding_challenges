...
function entryTime(s, keypad) {
    ...
    for (let i = 1, len = s.toString().length; i < len; i++){
      let num = s[i]
      let newPosition = map[num];
      let posXChange = Math.abs(position[0]-newPosition[0]);
      let posYChange = Math.abs(position[1]-newPosition[1]);
      if (posXChange === 1 && posYChange === 1) {
        movements += 1;
      } else if (posXChange >= 1 && posYChange >= 1) {
        movements += 2;
      }else {
        movements += posXChange + posYChange;
      }
